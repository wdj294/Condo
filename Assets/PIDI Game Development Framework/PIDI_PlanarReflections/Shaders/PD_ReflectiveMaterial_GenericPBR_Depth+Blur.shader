...
Shader "PIDI Shaders Collection/Planar Reflection/Generic/Depth+Blur" {
	Properties {
		[Space(12)]
		...
		_BlurSize( "Blur Size", Range( 0, 32 ) ) = 0 //The blur factor applied to the reflection
		...
		_ReflectionDepth("Reflection Depth", 2D) = "white"{}//Reflection depth
		_DepthBlurPower("Depth Blur Power", Range(0,1)) = 0.15
	...
	SubShader {
		...
		void surf (Input IN, inout SurfaceOutputStandardSpecular o) {
			...
			#endif
			float rDepth = length(tex2Dproj( _ReflectionDepth, UNITY_PROJ_COORD( IN.screenPos ) ).rgb);
			rDepth = pow( rDepth, 6*_DepthBlurPower);
			o.Smoothness = _Glossiness*gsoh.r; //Glossiness value is multiplied by the green channel of our GSOH texture
			
			half4 c = half4(0.0h,0.0h,0.0h,0.0h);    
			float depth = saturate(pow(rDepth, 2))*_BlurSize*0.0005; //Prepare the blur size
			
			if ( depth > 0 ){//If Blur level is bigger than one, we apply the blur effect. This makes the shader cheap when no blur is being applied since all this part will be skipped
			...
			o.Emission = e.rgb+lerp(1,1-e.a,_EmissionMode)*c.rgb*_ReflectionTint*lerp( _ReflectionTint.a*0.5, 1, 1-fresnelValue )*lerp( max( o.Albedo, half3( 0.1, 0.1, 0.1 ) ), half3( 1, 1, 1), 1-fresnelValue )*mColor.a;
