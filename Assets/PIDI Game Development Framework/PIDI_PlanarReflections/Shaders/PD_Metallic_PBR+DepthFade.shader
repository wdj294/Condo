Shader "PIDI Shaders Collection/Planar Reflection/PBR/Metallic Setup+Depth Fade" {
	...
	Properties
    {
        ...
        _SpecGlossMap("Gloss Map (R)", 2D) = "black" {}
		...
		_DepthBlurPower("Depth Fade Power", Range(0,1)) = 0.15
    }
	...
	SubShader {
		...
		void surf (Input IN, inout SurfaceOutputStandard o) {
			...
			o.Smoothness = lerp( _Glossiness, met.a, _MetSource );
			
			float rDepth = length(tex2Dproj( _ReflectionDepth, UNITY_PROJ_COORD( IN.screenPos ) ).rgb)*(o.Smoothness);
			...
			o.Emission = e.rgb+saturate(_DepthBlurPower-rDepth)*lerp(1,1-e.a,_EmissionMode)*c.rgb*_ReflectionTint*lerp( _ReflectionTint.a*0.5, 1, 1-fresnelValue )*lerp( max( o.Albedo, half3( 0.1, 0.1, 0.1 ) ), half3( 1, 1, 1), 1-fresnelValue );
			o.Alpha = c.a;
		}
		ENDCG
	}
